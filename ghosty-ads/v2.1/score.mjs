// Partitura de «Ghosty Ads — El colador» v2.1 (click-to-Messenger, vertical 1080×1920, ≤ 64 s).
// v2.1: dentro del acto «atiende», Ghosty también contesta los COMENTARIOS del anuncio y manda el DM.
...
const HOLD = 2.0, WIPE = 0.6, R = (x) => +x.toFixed(3);
const norm = (s) => s.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/[^a-z0-9]/g, "");

const DEF = [
  { id: "hook", lines: [["hookA", 0.3], ["hookB", 0.25]] },
  { id: "cmp", lines: [["cmpA", 0.4], ["cmpB", 0.25], ["cmpC", 0.25], ["cmpD", 0.4]] },
  { id: "atiende", lines: [["atiende", 0.4], ["comenta", () => (W("atiende", "agenda") + 0.3 + HOLD + 0.45) - (VOICE.atiende + L.atiende.dur)]] },
  { id: "filtro", lines: [["filtroA", 0.4], ["filtroB", 0.2]] },
  { id: "reporte", lines: [["reporteA", 0.4], ["reporteB", 0.2]] },
  { id: "cierre", lines: [["cierre", 0.35]] },
  { id: "cta", lines: [["ctaA", 0.4], ["ctaB", 0.2], ["ctaC", 0.25]], last: true },
];
const VOICE = {};
...
const ANIM = {
  ...
  cmp: (s) => ({
    ...
    toPause: VOICE.cmpD - 0.3, budget: VOICE.cmpD - 0.1, paused: W("cmpD", "pausa"), push: W("cmpD", "pausa", "e"), tap: W("cmpD", "prendes"), active: W("cmpD", "prendes") + 0.25,
  }),
  atiende: () => ({ reply: W("atiende", "contesta"), watch: W("atiende", "instante"), pdf: W("atiende", "cotiza"), cal: W("atiende", "agenda"), drop: W("atiende", "agenda") + 0.3,
                   toCom: VOICE.comenta - 0.45, k1: W("comenta", "comentan"), k2: W("comenta", "anuncio"), pub: W("comenta", "contesta"), plane: W("comenta", "escribe"), dm: W("comenta", "messenger") }),
  filtro: () => ({ b1: W("filtroA", "curiosos"), b2: W("filtroA", "despide"), bye: W("filtroA", "amabilidad"), stamp: W("filtroA", "gastar"),
                   gold: W("filtroA", "ellos"), beto: W("filtroB", "correo"), strike: W("filtroB", "falso"), flag: W("filtroB", "falso", "e") + 0.15 }),
  reporte: () => ({ clock: W("reporteA", "nueve"), sun: W("reporteA", "manana"), moon: W("reporteA", "noche"), push: W("reporteA", "reporte"),
                    row1: W("reporteB", "quien"), row2: W("reporteB", "busca"), row3: W("reporteB", "busca", "e") + 0.2 }),
  cierre: (s) => ({ fall: s.t0 + 0.3, gold: W("cierre", "hablas"), badge: W("cierre", "si") }),
...
for (const d of DEF) {
  let cur = t0;
  d.lines.forEach(([name, gap]) => { VOICE[name] = R(cur + (typeof gap === "function" ? gap() : gap)); cur = VOICE[name] + L[name].dur; });
  ...
  const t1 = d.last ? R(Math.max(voiceEnd + 1.6, lastAnim + 2.1)) : R(Math.max(voiceEnd + 0.9, lastAnim + HOLD + WIPE));
...
// comentarios del anuncio: burbujas, respuesta pública, avión de papel y el DM
fx(t("atiende.toCom"), "whoosh-short", 0.5); fx(t("atiende.k1"), "pop"); fx(t("atiende.k2"), "tick", 0.6); fx(t("atiende.pub"), "card"); fx(t("atiende.plane"), "whoosh-fly", 0.55); fx(t("atiende.dm"), "ding");
// colador
