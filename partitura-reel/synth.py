...
def pad(notes, t, dur):
    ...
    e = np.ones(len(x))
    at = int(0.02 * SR)
    rl = int(0.03 * SR)  # corte seco: el silencio lo escribe la partitura
    e[:at] = np.linspace(0, 1, at)
    e[-rl:] = np.linspace(1, 0, rl)
    place(y * trem * e * 0.05 / len(notes) * 3, t)


# ---------------- tocar ----------------
PAN = {"bell": -0.25, "marimba": 0.25, "pluck": 0.15, "click": 0.0}
for p in SCORE["pads"]:
    pad(p["notes"], p["t"], p["dur"])

for e in SCORE["events"]:
    inst, t, m, d, v = e["inst"], e["t"], e["note"], e["dur"], e["vel"]
    if inst == "bell":
        place(bell(m, d, v), t, PAN["bell"])
    elif inst == "marimba":
        place(marimba(m, d, v), t, PAN["marimba"])
    elif inst == "pluck":
        place(pluck(m, d, v), t, PAN["pluck"])
    elif inst == "kick":
        place(kick(v), t)
    elif inst == "hat":
        place(hat(v), t, 0.35)
...
print(f"{dst}: {DUR:.1f} s, {len(SCORE['events'])} eventos, pico normalizado a -1 dBFS")
